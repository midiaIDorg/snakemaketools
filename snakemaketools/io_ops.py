...
def expand_path(path: str) -> list[str]:
    """
    Expand bash curly bracket path into all paths.

    Autogenerated by ChatGPT. Modifided by MatteoLacki.
    """
    # Find all sets enclosed in curly braces
    sets = re.findall(r"\{([^}]+)\}", path)

    # Split each set by comma to get the list of elements
    lists = [s.split(",") for s in sets]

    # Get the Cartesian product of all sets
    combinations = list(itertools.product(*lists))

    # Generate all possible paths by replacing the placeholders in the original path
    for combo in combinations:
        new_path = path
        for val in combo:
            new_path = re.sub(r"\{[^}]+\}", val, new_path, count=1)
        yield new_path


def parse_path(
    path: str,
    _part_of_path_to_deleted: str = "out/base/",
) -> dict[str, str]:
    """Parse a path to get out all the values set in their file names."""
    path = path.replace(_part_of_path_to_deleted, "")
    path_parts = path.split("/")
    return {
        "config": path_parts[0],
        **dict(p.split("=") for p in path_parts if "=" in p),
    }
